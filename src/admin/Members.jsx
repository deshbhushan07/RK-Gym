...
import { getMembers, addMember, updateMember } from '../services/memberService';
...
export default function Members() {
  ...
  const getExpiry = (member) => {
    if (!member?.joinDate || !member?.plan) return null;
    const joinLocal = parseISO(member.joinDate);
    const daysToAdd = PLAN_DAYS[member.plan] || 30;
    return startOfDay(addDays(joinLocal, daysToAdd));
  };
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.phone || !form.joinDate) {
      toast.error('Name, phone, join date required');
      return;
    }
    setSaving(true);
    try {
  ...
  const handleRenew = async () => {
    if (!renewForm.fees) {
      toast.error('Enter renewal fees');
      return;
    }
    setRenewing(true);
    try {
      const today = format(new Date(), 'yyyy-MM-dd');
      const daysToAdd = PLAN_DAYS[renewForm.plan] || 30;
      const newExpiryDate = startOfDay(addDays(new Date(), daysToAdd));
      const newExpiry = format(newExpiryDate, 'dd MMM yyyy');

      await updateMember(renewTarget.id, {
        ...renewTarget,
        plan: renewForm.plan,
        fees: Number(renewForm.fees),
        joinDate: today,
        status: 'active',
      });
  ...
  const buildRows = () =>
    filtered.map(m => {
      ...
      const daysLeft = exp ? differenceInDays(exp, startOfDay(new Date())) : null;
  ...
  const filtered = members.filter(m => {
    ...
    if (filter === 'expired') {
      ...
      const exp = getExpiry(m);
      return match && exp && differenceInDays(exp, startOfDay(new Date())) < 0;
    }
  ...
  const ExpiryBadge = ({ member }) => {
    ...
    const d = differenceInDays(exp, startOfDay(new Date()));
    return (
          ...
          <div className="modal-box">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ background: 'var(--accent-dim)', border: '1px solid var(--border-accent)', borderRadius: 8, padding: '10px 14px', fontSize: '0.82rem', color: 'var(--accent)' }}>
                📅 New expiry: <strong>{format(startOfDay(addDays(new Date(), PLAN_DAYS[renewForm.plan] || 30)), 'dd MMM yyyy')}</strong>
