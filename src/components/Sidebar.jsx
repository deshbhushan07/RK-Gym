...
import {
  ...
  FiList, FiClipboard, FiCalendar, FiLogOut, FiZap,
...
// Bottom nav shows first 5, rest go in drawer
          ...
          <div style={ms.drawer} onClick={e => e.stopPropagation()}>
            ...
            <div style={ms.drawerBottom}>
              <div style={ms.drawerUser}>
                ...
                <div>
                  <div style={{ fontSize: '0.88rem', fontWeight: 600, color: 'var(--text-primary)' }}>Admin</div>
      ...
      <nav style={ms.bottomNav}>
        {bottomNav.map(item => (
          <NavLink key={item.to} to={item.to}
            ...
            <span style={ms.tabLabel}>{item.label.slice(0, 6)}</span>
          </NavLink>
        ))}
...
const ds = {
  ...
  brand: {
    ...
    padding: '0 14px 18px', borderBottom: '1px solid var(--border)', marginBottom: 10,
  },
  logo: { width: 32, height: 32, borderRadius: 8, background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#0a0a0a', fontSize: '1rem', flexShrink: 0 },
  logoName: { fontFamily: 'var(--font-display)', fontSize: '0.9rem', letterSpacing: '0.08em' },
  logoSub:  { fontSize: '0.62rem', color: 'var(--text-muted)', marginTop: 1 },
  nav: { flex: 1, display: 'flex', flexDirection: 'column', gap: 1, padding: '0 8px', overflowY: 'auto' },
  navItem: { display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 8, fontSize: '0.86rem', fontWeight: 500, color: 'var(--text-secondary)', transition: 'var(--transition)' },
  ...
  bottom: { padding: '14px 12px 0', borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 8 },
  userRow: { display: 'flex', alignItems: 'center', gap: 10, padding: '4px 6px' },
  userAvatar: { width: 32, height: 32, borderRadius: '50%', background: 'var(--accent-dim)', color: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontSize: '0.9rem', flexShrink: 0 },
  userInfo: { minWidth: 0 },
  ...
  logoutBtn: { display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderRadius: 8, width: '100%', fontSize: '0.85rem', fontWeight: 500, color: 'var(--text-secondary)', transition: 'var(--transition)' },
};

// ── Mobile styles ─────────────────────────────────────────
const ms = {
  bottomNav: { position: 'fixed', bottom: 0, left: 0, right: 0, height: 'var(--bottom-nav-height)', background: 'var(--bg-secondary)', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'stretch', zIndex: 200, paddingBottom: 'env(safe-area-inset-bottom)' },
  tab: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2, padding: '4px 2px', color: 'var(--text-muted)', transition: 'var(--transition)', background: 'none', border: 'none', cursor: 'pointer', WebkitTapHighlightColor: 'transparent' },
  tabActive: { color: 'var(--accent)' },
  tabIcon: { fontSize: '1.1rem', display: 'flex', lineHeight: 1 },
  tabLabel: { fontSize: '0.58rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.03em' },
  overlay: { position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', justifyContent: 'flex-end' },
  drawer: { width: 280, height: '100%', background: 'var(--bg-secondary)', borderLeft: '1px solid var(--border)', display: 'flex', flexDirection: 'column', paddingBottom: 'env(safe-area-inset-bottom)', overflowY: 'auto' },
  drawerHeader: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', borderBottom: '1px solid var(--border)' },
  ...
  logoName: { fontFamily: 'var(--font-display)', fontSize: '0.88rem', letterSpacing: '0.08em' },
  logoSub:  { fontSize: '0.6rem', color: 'var(--text-muted)' },
  ...
  drawerItem: { display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 10, fontSize: '0.92rem', fontWeight: 500, color: 'var(--text-secondary)', transition: 'var(--transition)', minHeight: 48 },
  drawerActive: { background: 'var(--accent-dim)', color: 'var(--accent)' },
  drawerIcon: { display: 'flex', fontSize: '1.1rem', flexShrink: 0 },
  drawerBottom: { padding: '16px', borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 10 },
